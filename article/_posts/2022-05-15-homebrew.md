...
Sollte es hier zu unübersichtlich werden, kann die Ausgabe auch in eine Text-Datei umgeleitet oder das ganze mit **grep* gefiltert werden.
...
Wie unter UX Systemen üblich, kann eine Anwendung auch diverse Abhängigkeiten haben. Um diese in einer "schönen" Übersicht zu haben, gibt es das folgende Kommando:
...
Um Abhängigkeiten zu entfernen, die von keiner anderen Anwenung benutzt wird, gibt es das Kommando:
...
Ich empfehle hier immer den Schalter `--dry-run` zu verwenden, denn bei allem was **remove** im Namen hat, bedeutet weg ist weg.
